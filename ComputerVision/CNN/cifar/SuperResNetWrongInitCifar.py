...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(self.dp(self.bn1(self.conv1(x))))
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(self.dp(self.bn1(self.conv1(x))))
        out = F.relu(self.dp(self.bn2(self.conv2(out))))
...
def TrainingModel(model, max_epochs, show_plots=False):
    loss_arr = []
    loss_epoch_arr = []
    train_acc_arr = []
    valid_acc_arr = []
    ...
    for epoch in trange(max_epochs):
        ...
        # Early stopping if there is no improvement in test accuracy for 10 epochs
    ...
    if show_plots:
        ...
        plt.savefig(
            f'../images/{model.name()}_CifarLoss.jpg', bbox_inches="tight")
        plt.clf()

        plt.ylabel("loss")
        plt.title('Loss across all batches')
        plt.plot(loss_arr)
        plt.savefig(f'../images/{model.name()}_CifarTLoss.jpg', bbox_inches="tight")
        plt.clf()
        ...
        plt.savefig(
            f'../images/{model.name()}_CifarAccuracy.jpg', bbox_inches="tight")
        plt.clf()

    torch.save(model.state_dict(), best)
    print('Train acc: %0.2f, Valid acc: %0.2f, Test acc: %0.2f' %
          (evaluation(model, trainloader), evaluation(model, validloader), evaluation(model, testloader)))
    return
...
trainloader, validloader, testloader = LoadDataloaders(name="resnet")
resnet = ResNet34().to(device)
PrintModelSummary(resnet, (3, 224, 224))
opt = optim.Adam(resnet.parameters(), lr=1e-5, weight_decay=1e-3)
lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    opt, mode='min', factor=0.5, patience=4)
TrainingModel(resnet, 100, show_plots=True)
end = time.time()
print(f'Total Time taken to train {resnet.name()} is {end-start} seconds')
