...
def TrainModel(epochs, show_plots=True):
    ...
    for epoch in trange(max_epochs):
        ...
        with torch.no_grad():
            ...
            if validacc > bestvalAcc:
                bestvalAcc = validacc
                bestepoch = epoch
                torch.save(BNDLeNet.state_dict(), "../checkpoints/LeNetBNDBest.pt")
        ...
        if epoch % 25 == 24:
            torch.save(BNDLeNet.state_dict(), "../checkpoints/LeNetBNDLast.pt")
    ...
    if show_plots:
        ...
        plt.savefig('../images/LeNetBND_Loss.jpg', bbox_inches="tight")
        plt.clf()
        ...
        plt.savefig('../images/LeNetBND_Accuracy.jpg', bbox_inches="tight")
        plt.clf()

    torch.save(BNDLeNet.state_dict(), "../checkpoints/LeNetBNDLast.pt")
    BNDLeNet.load_state_dict(torch.load("../checkpoints/LeNetBNDBest.pt"))
    print('Test acc: %0.2f, Valid acc: %0.2f, Train acc: %0.2f' % (evaluation(
        BNDLeNet, testloader), evaluation(BNDLeNet, validloader), evaluation(BNDLeNet, trainloader)))
    return
...
PrintModelSummary(BNDLeNet)
opt = optim.Adam(BNDLeNet.parameters(), lr=5*1e-5)
