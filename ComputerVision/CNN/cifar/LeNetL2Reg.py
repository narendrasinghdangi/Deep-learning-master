...
def TrainModel(epochs, show_plots=False):
    ...
    for epoch in trange(max_epochs):
        ...
        with torch.no_grad():
            ...
            if validacc > bestvalAcc:
                bestvalAcc = validacc
                bestepoch = epoch
                torch.save(net.state_dict(), "./checkpoints/LeNetL2RegBest.pt")
        ...
        if epoch % 25 == 24:
            torch.save(net.state_dict(), "./checkpoints/LeNetL2RegLast.pt")
    ...
    if show_plots:
        ...
        plt.savefig('./images/LeNetL2Reg_Loss.jpg', bbox_inches="tight")
        plt.clf()
        ...
        plt.savefig('./images/LeNetL2Reg_Accuracy.jpg', bbox_inches="tight")
        plt.clf()

    torch.save(net.state_dict(), "./checkpoints/LeNetL2RegLast.pt")
    net.load_state_dict(torch.load("./checkpoints/LeNetL2RegBest.pt"))
...
opt = optim.Adam(net.parameters(), lr=5*1e-5, weight_decay = 1e-3)
TrainModel(200, show_plots=True)
end = time.time()
print(f'Total Time taken to train LeNetL2Reg is {end-start} seconds')
